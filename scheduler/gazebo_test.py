...
class SchedulerNode(Node):
    ...
    def __init__(self) -> None:
        ...
        if not include_charge_param:
            self.get_logger().warn(
                "[scheduler] include_charge_in_objective=False is no longer supported; forcing optimized charging mode"
            )
        self.include_charge_in_objective = True

        default_variant = "with_charge"
        ...
        if seq_variant != "with_charge":
            self.get_logger().warn(
                f"[scheduler] sequence_execution_variant='{seq_variant_param}' is unsupported; using 'with_charge'"
            )
            seq_variant = "with_charge"
        ...
        self.get_logger().info(
            ...
            f"task allocation: running all algorithms and selecting best (epoch={self.optimization_epoch}, pop_size={self.optimization_pop_size})"
        )
    ...
    def _offline_initialize_sequences(self) -> None:
        ...
        # Store global path-planning (Nav2 precompute) time for logging alongside
        # per-algorithm optimization time in execution_time_log.txt
        self.path_planning_time_s = float(precompute_dt)
        if total_paths > 0:
        ...
        saved_charge_flag = self.include_charge_in_objective
        self.sequence_variants.clear()
        variants_wall = 0.0

        for variant_name, include_charge in variant_plan:
            self.include_charge_in_objective = include_charge
            variant_t0 = time.time()
            sequences, stats = self._compute_sequences_for_current_flag(candidate_tasks)
            stats["wall_time"] = time.time() - variant_t0
            variants_wall += stats["wall_time"]
            self.sequence_variants[variant_name] = {
            ...
            charges = stats.get("charge_events", 0)
            robots_used = stats.get("robots_utilized", 0)
            self.get_logger().info(
                f"[variant-{variant_name}] include_charge={include_charge} assigned_tasks={assigned} "
                f"charge_events={charges} robots={robots_used} stage1={stats['stage1_time']:.3f}s "
                f"energy={stats['energy_time']:.3f}s"
            )

        self.include_charge_in_objective = saved_charge_flag

        exec_variant = self.sequence_execution_variant
        if exec_variant not in self.sequence_variants:
            fallback = "with_charge" if "with_charge" in self.sequence_variants else next(iter(self.sequence_variants.keys()), None)
            self.get_logger().warn(
                f"[offline] Requested variant '{exec_variant}' not available, falling back to '{fallback}'"
            )
            exec_variant = fallback

        if exec_variant is None:
            self.get_logger().error("[offline] No sequence variants were generated; keeping sequences empty")
            self.optimization_in_progress = False
            return

        chosen_info = self.sequence_variants.get(exec_variant)
        if chosen_info is None:
            self.get_logger().warn("[offline] No sequence variants generated; keeping current_sequences empty")
            self.optimization_in_progress = False
            return

        self.include_charge_in_objective = chosen_info["include_charge"]
        self.active_sequence_variant = exec_variant
        sequences_to_use = chosen_info["sequences"]

        for rid in self.robot_ids:
            final_seq = sequences_to_use.get(rid, [])
            ...
            self.get_logger().info(
                f"[offline:{exec_variant}] initialized sequence for {rid}: "
                f"tasks={seq_ids} (total_len={len(seq_ids)}, tasks={len(non_charge_tasks)}, charges={charge_count})"
            )

        other_variants = [k for k in self.sequence_variants.keys() if k != exec_variant]
        if other_variants:
            self.get_logger().info(
                f"[offline] Stored additional sequence variants for analysis: {other_variants}"
            )
        ...
        self.optimization_time = total_optimization_dt
        self.get_logger().info(
            f"[optimization] Total optimization time: {total_optimization_dt:.3f}s "
            f"(precompute: {precompute_dt:.3f}s, variants: {variants_wall:.3f}s)"
        )
        
        self.get_logger().info(
            f"[optimization] Allocation complete using variant '{exec_variant}' - energy consumption resumed"
        )
