...
def plot_one_timeline_on_ax(
    ...
    def simulate_with_scales(scales: Dict[str, float]) -> Tuple[
        ...
        def advance_until_charge(robot: str) -> Optional[Dict[str, Any]]:
            ...
            while st["idx"] < len(seq):
                ...
                main_time = float(task.get("total_time", 0.0))
    ...
    for r, label in zip(robots, display_labels):
        ...
        msg = f"  {label}: makespan={makespan_by_robot[r]:.3f}s energy_used(task+move)={energy_used:.3f}Wh"
        print(msg)
...
def make_6_timelines_figure(
    ...
    for ax, case in zip(axes, cases):
        totals = plot_one_timeline_on_ax(
            ax=ax,
            tasks=tasks,
    ...
    # compute global max makespan from provided targets (most reliable)
    global_max_ms = 0.0
    for case in cases:
        tm = case.get("target_makespan", {})
        if tm:
            global_max_ms = max(global_max_ms, max(float(v) for v in tm.values()))

    # fallback (if target_makespan is missing): keep autoscale
    if global_max_ms > 0.0:
        for ax in axes:
            ax.set_xlim(0.0, global_max_ms)
...
if __name__ == "__main__":
    ...
    cases.append({
        "algo_name": "PSO",
        "robot_sequences": {
            "tb1": [46, 30, "c2", 6, 41, 44, 7, 36, "c2", 22, 47, 49],
            "tb2": [48, 27, 35, 18, 32, "c1", 26, 31, "c1", 19, 16, 1, "c1", 14, 5, 50],
            "tb3": [37, 42, 9, 28, "c1", 25, 17, 45, 2, "c1", 33, 13, 29, 12, 43],
    ...
    cases.append({
        ...
        "robot_sequences": {
            ...
            "tb2": [16, 26, "c1", 38, 8, 24, 39, "c2", 41, 20, 40, 23, 12, 2, 28, 32],
    ...
    cases.append({
        "algo_name": "FLA",
        "robot_sequences": {
            "tb1": [24, "c1", 3, 43, 13, "c2", 14, 26, 8, 48, "c1", 10, 16, 15, 6, 23, 18, 9],
            "tb2": [50, 25, "c2", 17, 22, 38, "c1", 5, 19, 27, 20, 34, 49, 7, 29, 12],
    ...
    cases.append({
        ...
        "robot_sequences": {
            ...
            "tb2": [10, 32, 5, 4, 39, "c2", 6, 46, 11, 13, 33, 45, 47],
