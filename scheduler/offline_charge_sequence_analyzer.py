...
충전을 고려한 optimized + threshold + feasibility 전략을 동시에 분석하는 스크립트.

사용법:
1. DECODE_OPTIMIZED_TEXT 에 sequence_log.txt 의 [decode-RIME] 라인을 복붙 (DECODE_NO_CHARGE_TEXT 는 legacy 용)
...
LOG_SEQUENCE_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "log_sequence")
os.makedirs(LOG_SEQUENCE_DIR, exist_ok=True)

DEFAULT_TIMELINE_PATH = os.path.join(LOG_SEQUENCE_DIR, "timeline_offline.png")
DEFAULT_CHARGE_TIMELINE_PATH = os.path.join(LOG_SEQUENCE_DIR, "timeline_charge.png")


ACTIVE_STRATEGIES = ["threshold", "feasibility", "optimized"]


def _ordered_present_strategies(strategies: Iterable[str]) -> List[str]:
    present = set(strategies)
    ordered = [name for name in ACTIVE_STRATEGIES if name in present]
    extras = sorted(present - set(ACTIVE_STRATEGIES))
...
def simulate_robot(
    ...
    for task_idx, task in enumerate(seq):
        ...
        if insert_charge and task.task_type != "CHARGE":
            ...
            need_charge = False
            threshold_limit = cap_wh * threshold_frac
            if strategy == "threshold":
                if energy < threshold_limit:
                    need_charge = True
...
[decode-RIME] tb1 sequence=[42,41,48,39,28,32,49,30,15,13,31] (length=11, tasks=[DUAL_PICK_MULTI_DROP:4, MULTI_PICK_DROP:5, PICK_DROP:2], charges=0, makespan=789.80)
[decode-RIME] tb2 sequence=[3,10,34,33,1,50,46,6,22,18,2,43,19] (length=13, tasks=[DUAL_PICK_MULTI_DROP:3, MULTI_PICK_DROP:3, PICK_DROP:7], charges=0, makespan=809.93)
[decode-RIME] tb3 sequence=[4,27,20,24,21,44,5,35,47,23,45,9,40] (length=13, tasks=[DUAL_PICK_MULTI_DROP:5, MULTI_PICK_DROP:4, PICK_DROP:4], charges=0, makespan=813.09)
[decode-RIME] tb4 sequence=[25,36,12,8,38,11,37,14,26,7,29,17,16] (length=13, tasks=[DUAL_PICK_MULTI_DROP:3, MULTI_PICK_DROP:3, PICK_DROP:7], charges=0, makespan=799.07)
...
[decode-RIME] tb1 sequence=[15,7,41,29,-1,8,20,39,-1,4,-1,33,27,47,5,14] (length=16, tasks=[DUAL_PICK_MULTI_DROP:3, MULTI_PICK_DROP:3, PICK_DROP:7], charges=3, makespan=1645.03)
[decode-RIME] tb2 sequence=[9,21,-1,18,49,34,17,45,-1,24,-1,2,40,22,38,31] (length=16, tasks=[DUAL_PICK_MULTI_DROP:5, MULTI_PICK_DROP:4, PICK_DROP:4], charges=3, makespan=1783.78)
[decode-RIME] tb3 sequence=[11,42,6,-1,13,12,50,1,-1,23,35,30,44] (length=13, tasks=[DUAL_PICK_MULTI_DROP:3, MULTI_PICK_DROP:3, PICK_DROP:5], charges=2, makespan=1181.09)
[decode-RIME] tb4 sequence=[46,43,37,36,-1,10,19,25,-1,28,32,3,16,26,48] (length=15, tasks=[DUAL_PICK_MULTI_DROP:4, MULTI_PICK_DROP:5, PICK_DROP:4], charges=2, makespan=1765.04)
...
def run_analysis(
    ...
    baseline_seqs, _ = parse_decode_text(DECODE_NO_CHARGE_TEXT)
    optimized_logged_ms = parse_decode_makespan(DECODE_OPTIMIZED_TEXT)

    strategy_map = {
        "optimized": {
            "seqs": optimized_seqs,
            "charger_choices": optimized_charger_choices,
            "insert_charge": False,
        },
        "threshold": {
            "seqs": baseline_seqs,
    ...
    strategy_entries: List[Tuple[str, Dict[str, object]]] = []
    for strat in ACTIVE_STRATEGIES:
        cfg = strategy_map.get(strat)
        seqs = cfg.get("seqs") if cfg else None
        if not cfg or not isinstance(seqs, dict) or not seqs:
            continue
        strategy_entries.append((strat, cfg))

    if not strategy_entries:
        print("[WARN] run_analysis: no strategies to evaluate (check DECODE_* text blocks)")
        return [], {}, {}

    for strat, _cfg in strategy_entries:
        PHASE1_TASK_TIMELINE[strat] = {}

    all_results: List[SimResult] = []
    timeline_events_by_strategy: Dict[str, List[Tuple[str, int, int, float, float, float]]] = {}
    phase1_charge_ends_by_strategy: Dict[str, Dict[str, List[float]]] = {}

    for strat, cfg in strategy_entries:
        seqs: Dict[str, List[int]] = cfg["seqs"]  # type: ignore[assignment]
        charger_choices_map: Dict[str, List[int]] = cfg["charger_choices"]  # type: ignore[assignment]
        insert_charge = bool(cfg.get("insert_charge", False))

        robot_ids = sorted(seqs.keys())
        if not robot_ids:
            continue
        ...
        for rid in robot_ids:
            ...
            init_pos = ROBOT_INITIAL_POSITIONS.get(rid, (0.0, 0.0))
            seq = build_task_sequence(task_ids, CHARGERS)
            charger_choices = charger_choices_map.get(rid)
            ...
            res, evs = simulate_robot(
                ...
                path_node=path_node,
                strategy=strat,
                insert_charge=insert_charge,
                charger_choices=charger_choices,
            )
            phase1_results[rid] = res
            events_all.extend(evs)
...
def print_results(results: List[SimResult]) -> None:
    ...
    for rid in sorted(by_robot.keys()):
        print(f"\n[Robot {rid}]")
        ordered = _ordered_present_strategies(by_robot[rid].keys())
    ...
    ordered_global = _ordered_present_strategies(by_strat.keys())
    for strat in ordered_global:
...
def _results_to_temp_cases(results: List[SimResult]) -> List[Dict[str, object]]:
    ...
    for strat in _ordered_present_strategies(by_strat.keys()):
...
def generate_timeline_png_from_results(
    ...
    """temp.py 시각화 엔진을 사용해 optimized 타임라인 PNG 생성."""
...
def generate_internal_timeline_png(
    ...
    # 시각화 순서: optimized, 그 다음 6개 알고리즘은 PSO→WCA→QSA→SMA→FLA→RIME
    fixed_order = ACTIVE_STRATEGIES + ["PSO", "WCA", "QSA", "SMA", "FLA", "RIME"]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Offline charge sequence analyzer (optimized / threshold / feasibility)")
    ...
    parser.add_argument("--multi-algo-out-dir", default=LOG_SEQUENCE_DIR, help="--multi-algo 시 PNG 저장 디렉터리 (default: log_sequence)")
    ...
    parser.add_argument("--timeline-out", default=DEFAULT_TIMELINE_PATH, help="생성할 타임라인 PNG 파일 경로 (default: log_sequence/timeline_offline.png)")
    ...
    parser.add_argument("--charge-timeline-out", default=DEFAULT_CHARGE_TIMELINE_PATH, help="충전 구간만 표시한 타임라인 PNG 파일 경로 (default: log_sequence/timeline_charge.png)")
    ...
    if args.multi_algo:
        try:
            run_multi_algorithm_timelines(
                path_node,
                DECODE_OPTIMIZED_TEXT,
                out_dir=os.path.abspath(args.multi_algo_out_dir),
            )
        finally:
    ...
    try:
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        save_results(results, os.path.join(LOG_SEQUENCE_DIR, f"charging_analysis_{timestamp}.csv"))
