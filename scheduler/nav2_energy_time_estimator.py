...
def _task_start_pose(task: TaskSpec) -> Tuple[float, float]:
    if task.picks:
        return task.picks[0]
    if task.drops:
        return task.drops[0]
...
def compute_demo_task_metrics(path_node: PathQueryNode) -> List[Dict[str, object]]:
    metrics: List[Dict[str, object]] = []
    default_soc = ROBOT_CAP_WH.get("tb1", max(ROBOT_CAP_WH.values(), default=60.0))
    for tid in sorted(TASKS.keys()):
        task = TASKS[tid]
        start_x, start_y = _task_start_pose(task)
        robot = RobotState(
            robot_id="tb1",
            x=start_x,
            y=start_y,
        ...
        energy_wh, duration_s = estimate_task_energy(robot, task, path_node)
        metrics.append({
            "task_id": tid,
...
def write_task_metrics(metrics: List[Dict[str, object]], file_path: str) -> None:
    os.makedirs(os.path.dirname(file_path) or ".", exist_ok=True)
    with open(file_path, "w") as f:
        f.write("task_id,task_type,makespan_s,energy_wh\n")
        for entry in metrics:
            f.write(
                f"{int(entry['task_id'])},{entry['task_type']},{entry['makespan_s']:.2f},{entry['energy_wh']:.2f}\n"
            )
...
DECODE_NO_CHARGE_TEXT = """
 [decode-RIME] tb1 sequence=[43,10,27,42,46,4,1,22,26,31,47,37] (length=12, tasks=[DUAL_PICK_MULTI_DROP:4, MULTI_PICK_DROP:5, PICK_DROP:3], charges=0, makespan=822.73)
 [decode-RIME] tb2 sequence=[16,21,15,12,41,49,35,29,17,48,30,13,6] (length=13, tasks=[DUAL_PICK_MULTI_DROP:4, MULTI_PICK_DROP:3, PICK_DROP:6], charges=0, makespan=821.12)
 [decode-RIME] tb3 sequence=[34,25,11,3,9,24,28,19,32,8,2,18,40] (length=13, tasks=[DUAL_PICK_MULTI_DROP:5, MULTI_PICK_DROP:1, PICK_DROP:7], charges=0, makespan=822.75)
 [decode-RIME] tb4 sequence=[23,38,50,33,36,45,39,14,5,20,44,7] (length=12, tasks=[DUAL_PICK_MULTI_DROP:2, MULTI_PICK_DROP:6, PICK_DROP:4], charges=0, makespan=798.26)
...
[decode-PSO] tb1 sequence=[46,30,-1,6,41,44,7,36,-1,22,47,49] (length=12, tasks=[DUAL_PICK_MULTI_DROP:2, MULTI_PICK_DROP:6, PICK_DROP:2], charges=2, makespan=1740.85)
[decode-PSO] tb2 sequence=[48,27,35,18,32,-1,26,31,-1,19,16,1,-1,14,5,50] (length=16, tasks=[DUAL_PICK_MULTI_DROP:5, MULTI_PICK_DROP:2, PICK_DROP:6], charges=3, makespan=1739.56)
[decode-PSO] tb3 sequence=[37,42,9,28,-1,25,17,45,2,-1,33,13,29,12,43] (length=15, tasks=[DUAL_PICK_MULTI_DROP:4, MULTI_PICK_DROP:4, PICK_DROP:5], charges=2, makespan=1465.91)
[decode-PSO] tb4 sequence=[20,15,34,24,11,40,38,-1,10,39,3,21,4,8,23] (length=15, tasks=[DUAL_PICK_MULTI_DROP:4, MULTI_PICK_DROP:3, PICK_DROP:7], charges=1, makespan=1550.86)

[decode-WCA] tb1 sequence=[33,34,7,-1,48,44,1,25,-1,31,5,30,11,14,15] (length=15, tasks=[DUAL_PICK_MULTI_DROP:5, MULTI_PICK_DROP:2, PICK_DROP:6], charges=2, makespan=1427.27)
[decode-WCA] tb2 sequence=[16,26,-1,38,8,24,39,-1,41,20,40,23,12,2,28,32] (length=16, tasks=[DUAL_PICK_MULTI_DROP:5, MULTI_PICK_DROP:4, PICK_DROP:5], charges=2, makespan=1434.73)
[decode-WCA] tb3 sequence=[46,10,-1,47,19,27,43,21,36] (length=9, tasks=[DUAL_PICK_MULTI_DROP:2, MULTI_PICK_DROP:4, PICK_DROP:2], charges=1, makespan=721.00)
[decode-WCA] tb4 sequence=[35,13,6,22,45,18,-1,37,3,50,4,49,9,42,17,29] (length=16, tasks=[DUAL_PICK_MULTI_DROP:3, MULTI_PICK_DROP:5, PICK_DROP:7], charges=1, makespan=1273.18)

[decode-QSA] tb1 sequence=[14,36,7,-1,30,8,-1,18,15,41,19,-1,37,1,-1,42,31,46,34] (length=19, tasks=[DUAL_PICK_MULTI_DROP:3, MULTI_PICK_DROP:5, PICK_DROP:7], charges=4, makespan=1708.02)
[decode-QSA] tb2 sequence=[10,16,20,38,-1,48,29,22,-1,26,13,28,9,3,32] (length=15, tasks=[DUAL_PICK_MULTI_DROP:5, MULTI_PICK_DROP:2, PICK_DROP:6], charges=2, makespan=1698.67)
[decode-QSA] tb3 sequence=[4,6,45,12,-1,40,43,-1,25,50,44,11] (length=12, tasks=[DUAL_PICK_MULTI_DROP:1, MULTI_PICK_DROP:5, PICK_DROP:4], charges=2, makespan=1207.35)
[decode-QSA] tb4 sequence=[49,35,17,39,21,-1,2,27,24,5,33,23,47] (length=13, tasks=[DUAL_PICK_MULTI_DROP:6, MULTI_PICK_DROP:3, PICK_DROP:3], charges=1, makespan=1461.49)

[decode-SMA] tb1 sequence=[38,18,20,-1,47,2,17,41,3,-1,14,48,-1,21,43,32,9] (length=17, tasks=[DUAL_PICK_MULTI_DROP:2, MULTI_PICK_DROP:5, PICK_DROP:7], charges=3, makespan=1414.22)
[decode-SMA] tb2 sequence=[40,44,50,-1,46,31,8,16,39,30,34] (length=11, tasks=[DUAL_PICK_MULTI_DROP:3, MULTI_PICK_DROP:5, PICK_DROP:2], charges=1, makespan=1105.35)
[decode-SMA] tb3 sequence=[15,7,36,26,-1,45,6,27,25,-1,11,12,37,35,1] (length=15, tasks=[DUAL_PICK_MULTI_DROP:4, MULTI_PICK_DROP:3, PICK_DROP:6], charges=2, makespan=1448.62)
[decode-SMA] tb4 sequence=[24,42,13,23,22,29,-1,28,10,49,5,33,4,19] (length=14, tasks=[DUAL_PICK_MULTI_DROP:6, MULTI_PICK_DROP:2, PICK_DROP:5], charges=1, makespan=1442.84)

[decode-FLA] tb1 sequence=[24,-1,3,43,13,-1,14,26,8,48,-1,10,16,15,6,23,18,9] (length=18, tasks=[DUAL_PICK_MULTI_DROP:3, MULTI_PICK_DROP:2, PICK_DROP:10], charges=3, makespan=1502.60)
[decode-FLA] tb2 sequence=[50,25,-1,17,22,38,-1,5,19,27,20,34,49,7,29,12] (length=16, tasks=[DUAL_PICK_MULTI_DROP:5, MULTI_PICK_DROP:3, PICK_DROP:6], charges=2, makespan=1432.61)
[decode-FLA] tb3 sequence=[11,36,-1,32,39,35,-1,33,46,44,42] (length=11, tasks=[DUAL_PICK_MULTI_DROP:3, MULTI_PICK_DROP:5, PICK_DROP:1], charges=2, makespan=1358.05)
[decode-FLA] tb4 sequence=[30,2,28,40,-1,1,37,41,45,21,31,4,47] (length=13, tasks=[DUAL_PICK_MULTI_DROP:4, MULTI_PICK_DROP:5, PICK_DROP:3], charges=1, makespan=1115.80)




[decode-OPT][RIME] tb1 sequence=[35,17,-1,44,36,23,14,-1,22,38,-1,50,43,21,49,20]
[decode-OPT][RIME] tb2 sequence=[10,32,5,4,39,-1,6,46,11,13,33,45,47]
[decode-OPT][RIME] tb3 sequence=[19,3,-1,24,-1,27,25,48,9,-1,28,34,31,7,41]
[decode-OPT][RIME] tb4 sequence=[8,15,42,37,-1,30,2,40,29,26,12,1,18,16]
...
def main() -> None:
    ...
    try:
        ...
        metrics = compute_demo_task_metrics(path_node)
        metrics_dir = os.path.dirname(args.timeline_out) or LOG_DIR
        os.makedirs(metrics_dir, exist_ok=True)
        metrics_path = os.path.join(metrics_dir, "task_nav2_metrics.txt")
        write_task_metrics(metrics, metrics_path)
        print(f"[Saved] per-task Nav2 metrics: {metrics_path}")
