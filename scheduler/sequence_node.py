...
from geometry_msgs.msg import PoseWithCovarianceStamped, PoseStamped, TransformStamped, Pose, Twist
...
class SchedulerNode(Node):
    ...
    def __init__(self) -> None:
        ...
        # cmd_vel 리미터: Nav2 출력을 받아 평소 0.3 m/s, 운반 중 0.25 m/s로 제한해 로봇에 전달
        self.cmd_vel_pubs: Dict[str, rclpy.publisher.Publisher] = {}
        ...
        # PICK_DROP, DUAL_PICK_MULTI_DROP : 1개 8kg
        # MULTI_PICK_DROP: 2개 집을 때 첫 번째 8kg, 두 번째 4kg → 총 12kg
        ...
        self.optimization_epoch = self.declare_parameter("optimization_epoch", 500).get_parameter_value().integer_value
        self.optimization_pop_size = self.declare_parameter("optimization_pop_size", 100).get_parameter_value().integer_value
        ...
        # For this run, default charging strategy to 'feasibility'
        charge_param = self.declare_parameter("charge", "feasibility").get_parameter_value().string_value
        ...
        # For this run, do not include charging in the optimization objective
        include_charge_param = self.declare_parameter("include_charge_in_objective", False).get_parameter_value().bool_value
        self.include_charge_in_objective = include_charge_param
        
        # Enable/disable Nav2 recovery on task failure
        # - True: Perform full Nav2 recovery (cancel goals, clear costmaps, reset TF)
        # - False: Skip recovery, just reassign task immediately
        self.enable_nav2_recovery = self.declare_parameter("enable_nav2_recovery", True).get_parameter_value().bool_value
        ...
        self.get_logger().info(
            f"[scheduler] Nav2 recovery: {'enabled' if self.enable_nav2_recovery else 'disabled'}"
        )

        # Gazebo spawn/delete/set_entity_state clients for cargo box visualization (운반 시 로봇에 붙어 따라감)
        if HAS_GAZEBO_MSGS:
            self.spawn_entity_client = self.create_client(SpawnEntity, '/spawn_entity')
            self.delete_entity_client = self.create_client(DeleteEntity, '/delete_entity')
            self.get_logger().info("[scheduler] Gazebo clients created (spawn/delete for task_id; cargo는 로봇 SDF cargo_visual 링크 사용)")
        else:
            self.spawn_entity_client = None
            self.delete_entity_client = None
            self.get_logger().warn("[scheduler] gazebo_msgs not found - box visualization disabled")
        
        # Track spawned task_id models in Gazebo (robot_id -> model_name)
        self.gazebo_task_models: Dict[str, str] = {}
        
        # Cargo visibility disabled - will re-enable with simpler approach later
        # self._cargo_visible_pubs: Dict[str, Dict[str, Any]] = {}
        # self._cargo_visible_last: Dict[str, Dict[str, bool]] = {}
        # for rid in self.robot_ids:
        #     self._cargo_visible_pubs[rid] = {
        #         "big": self.create_publisher(Bool, f"/{rid}/cargo_visible_big", 10),
        #         "small": self.create_publisher(Bool, f"/{rid}/cargo_visible_small", 10),
        #     }
        #     self._cargo_visible_last[rid] = {"big": False, "small": False}
        # # 초기: 모두 숨김
        # for rid in self.robot_ids:
        #     for key in ("big", "small"):
        #         msg = Bool()
        #         msg.data = False
        #         self._cargo_visible_pubs[rid][key].publish(msg)
        ...
        # Periodic robot task_id display timer (every 3 seconds)
        self.task_id_display_timer = self.create_timer(3.0, self._task_id_display_timer_cb)
        ...
        # ------------------------------------------------------------------
        # 수동 시퀀스 모드: 사용자가 로봇별 task_id 배열을 직접 지정해
        # 알고리즘 최적화 없이 그 순서대로 실행하도록 하는 옵션.
        # 형식(JSON 문자열 예): {"tb1":[1,4,7], "tb2":[2,3]}
        # ------------------------------------------------------------------
        self.manual_sequences_enabled = False
        manual_seq_json = self.declare_parameter("manual_sequence_json", "").get_parameter_value().string_value
        manual_seq_map: Dict[str, List[Any]] = {}
        if manual_seq_json:
            try:
                parsed = json.loads(manual_seq_json)
                if isinstance(parsed, dict):
                    manual_seq_map = parsed
                else:
                    self.get_logger().warn("[manual-seq] manual_sequence_json는 dict 형식이어야 합니다 (robot_id -> [task_id,...])")
            except Exception as e:
                self.get_logger().warn(f"[manual-seq] manual_sequence_json 파싱 실패: {e}")
        ...
        # ------------------------------------------------------------------
        # Offline scheduling OR 수동 시퀀스 모드
        # ------------------------------------------------------------------
        if manual_seq_map and self._apply_manual_sequences(manual_seq_map):
            self.manual_sequences_enabled = True
            self.get_logger().info("[manual-seq] 알고리즘 최적화 없이 수동 시퀀스를 사용합니다.")
        else:
            self.manual_sequences_enabled = False
    ...
    def _save_execution_time_log(self, algorithm_name: str, execution_time: float):
        """Save algorithm execution time log to file."""
        try:
            with open(self.execution_time_log_file, 'a') as f:
                f.write(f"[execution-time-{algorithm_name}] Execution time = {execution_time:.3f}s\n")
        except Exception as e:
                ...
                if gazebo_gt is None:
                    ...
                    try:
                        ...
                        odom_to_base = self.tf_buffer.lookup_transform(
                            base_frame, odom_frame, rclpy_time.Time(), timeout=rclpy_duration.Duration(seconds=0.5)
                        )
    ...
    def estimate_task_energy(self, robot: RobotState, task: TaskSpec):
        ...
        if task.task_type == "MULTI_PICK_DROP":
            ...
            for i, (px, py) in enumerate(picks):
                ...
                # determine picked item weight from pick_weights or task type
                w = 0.0
                try:
                    ...
                    w = 0.0
                if w <= 0.0:
                    w = self.get_item_weight(task.task_type, i)
        ...
        for i in range(n):
            ...
            if i < len(picks):
                ...
                # Add item weight if provided, otherwise use task type weight
                w = 0.0
                try:
                    w = float(task.pick_weights[i]) if (hasattr(task, 'pick_weights') and i < len(task.pick_weights)) else 0.0
                except Exception:
            ...
            if i < len(drops):
                ...
                # the weight of the item that was picked for this drop
                dropped_weight = self.get_item_weight(task.task_type, i)
            ...
            for task in seq:
                ...
                if soc < required:
                    ...
                    if charger is None:
                        ...
                        # Charge duration proportional to remaining capacity at this point
    ...
    def _init_demo_tasks(self) -> None:
        ...
        for i in range(len(triplets)):
            ...
            # per-pick weights: 8kg + 4kg (task_type_weights와 동일)
            pick_weights = list(self.task_type_weights.get("MULTI_PICK_DROP", [8.0, 4.0]))
        ...
        for i in range(len(triplets)):
            ...
            # per-pick weights: 8kg(task_type_weights와 동일)
            pick_weights = list(self.task_type_weights.get("DUAL_PICK_MULTI_DROP", [8.0, 8.0]))
            # deliveries: first drop receives pick index 0, second drop receives pick index 1
            deliveries = [[0], [1]]
        ...
        for task_data in all_tasks:
            ...
            if task_type == "PICK_DROP":
                px, py = picks_or_pick
                dx, dy = drops_or_drop
                # PICK_DROP: 1개 8kg (task_type_weights와 동일)
                default_weight = self.task_type_weights.get("PICK_DROP", [8.0])[0]
    ...
    def _optimize_with_algorithm(self, candidate_tasks: List[TaskSpec], algorithm_name: str) -> Tuple[Dict[str, List[TaskSpec]], float, Optional[float], Optional[float]]:
        ...
        # ---------- objective ----------
        # Storage for max_makespan and total energy from best solution evaluation
        ...
        store_makespan_flag = [False]  # Flag to control when to store makespan
        ...
        def _objective_function(solution: np.ndarray) -> float:
            ...
            for rid in robot_list:
                ...
                for task_idx, t in enumerate(seq):
                    ...
                    if should_charge:
                        ...
                        # ------------------------------------------------------------------
                        # Dynamic charge duration based on remaining SOC
                        # remaining_frac: fraction of capacity needed to reach full charge
                        # charge_rate_frac_per_sec: fraction of capacity charged per second
                        # ------------------------------------------------------------------
                        remaining_frac = max(0.0, (cap_wh - energy) / cap_wh) if cap_wh > 0.0 else 0.0
                        charge_rate_frac_per_sec = 0.1
                        if remaining_frac <= 0.0:
                            charge_duration = 0.0
                        else:
            ...
            # Store max_makespan, total energy, and robot_times only when flag is set (for best solution evaluation)
            ...
            if include_charge_flag:
                ...
                total_cost = (
                    max_makespan           # 원래는 normalized_makespan, 지금은 정규화 제외 
                    + 0.3 * sum_robot_times  # 이동/우회/대기 해결  
                    + 100.00 * total_charge_count # 충전 남발 억제
                    + 1e5 * early_charge_count # 충전 조기 억제
                    + total_penalty
                )
            else:
        ...
        # Re-evaluate best solution to extract max_makespan, total energy, and per-robot makespans
        ...
        # Return decoded assignment, best fitness, max makespan, and total energy
        return result, best_fitness, best_solution_makespan, best_solution_energy
    ...
    def _optimize_multi_robot_allocation(self, candidate_tasks: List[TaskSpec]) -> Dict[str, List[TaskSpec]]:
        ...
        # For this experiment, only run a single round.
        total_rounds = 5
        ...
        for round_num in range(total_rounds):
            ...
            for algo_name in algorithms:
                try:
                    ...
                    run_t0 = time.time()
                    result, fitness, makespan, energy = self._optimize_with_algorithm(candidate_tasks, algo_name)
        ...
        for algo_name in algorithms:
            ...
            self._save_execution_time_log(algo_name, total_time_per_algo[algo_name])
        ...
        self._save_multi_round_statistics_log(
            total_rounds, algorithms, all_fitnesses_per_algo, all_makespans_per_algo, all_energies_per_algo,
            best_fitness_per_algo, best_makespan_per_algo, best_energy_per_algo, total_time_per_algo,
            global_best_algo, global_best_round, global_best_fitness, global_best_makespan, global_best_energy
        )
        
        return global_best_result

    def _apply_manual_sequences(self, manual_seq_map: Dict[str, List[Any]]) -> bool:
        """사용자 지정 시퀀스를 그대로 적용한다.

        Args:
            manual_seq_map: {"tb1":[1,4,-1,7], "tb2":[2,3,-1], ...}
                           task_id: 정수 (CHARGE 태스크)
                           -1: 자동으로 가장 가까운 charger로 CHARGE 태스크 생성.
        Returns:
            bool: 한 로봇 이상에 유효 시퀀스가 적용되면 True.
        """
        if not manual_seq_map:
            return False
        ...
        for rid in self.robot_ids:
            seq_ids = manual_seq_map.get(rid, [])
            seq_specs: List[TaskSpec] = []

            for sid in seq_ids:
                if isinstance(sid, (int, float)) and int(sid) == sid:
                    tid = int(sid)
                    
                    # -1 이면 CHARGE 태스크 생성
                    if tid == -1:
                        # 로봇의 현재 위치에서 가장 가까운 charger로 CHARGE 태스크 생성
                        state = self.robot_states.get(rid)
                        if state is None:
                            self.get_logger().warn(f"[manual-seq] {rid}: CHARGE 태스크 생성 실패 (robot state 없음)")
                            continue
                        
                        charger = self._nearest_charger(state.x, state.y)
                        charge_task = TaskSpec(
                            task_id=-1,  # CHARGE 태스크는 task_id=-1
                            task_type="CHARGE",
                            picks=[(charger.x, charger.y)],
                            drops=[(charger.x, charger.y)],
                            charge_duration_s=30.0,
                        )
                        seq_specs.append(charge_task)
                    else:
                        spec = task_lookup.get(tid)
                        if spec is None:
                            self.get_logger().warn(f"[manual-seq] {rid}: task_id={tid} 가 존재하지 않아 건너뜁니다.")
                            continue
                        seq_specs.append(copy.deepcopy(spec))
                else:
                    self.get_logger().warn(f"[manual-seq] {rid}: 지원하지 않는 토큰 sid={sid!r} (정수 task_id와 -1만 허용)")

            self.current_sequences[rid] = seq_specs
            self.current_seq_index[rid] = 0

            if seq_specs:
                applied = True
                seq_str = ",".join(str(t.task_id) if t.task_id != -1 else "CHARGE" for t in seq_specs)
                # task 타입 카운트
                ...
                self.get_logger().info(
                    f"[manual-seq] {self._color_robot(rid)} 적용된 시퀀스: [{seq_str}] "
                    f"(len={len(seq_specs)}, charges={charge_count})"
                )
                # 로그 파일에도 남긴다 (makespan 미계산 → None)
                try:
                    self._save_sequence_log("manual", rid, seq_str, len(seq_specs), type_counts, charge_count, None)
                except Exception as e:
                    self.get_logger().warn(f"[manual-seq] sequence log 저장 실패: {e}")
            else:
                self.get_logger().warn(f"[manual-seq] {self._color_robot(rid)} 시퀀스가 비어 있습니다.")
    ...
    def _init_robot_interfaces(self, robot_id: str) -> None:
        ...
        # cmd_vel 리미터: velocity_smoother 출력(cmd_vel_limiter_input)을 받아 평소 0.3, 운반 중 0.25로 제한해 cmd_vel로 발행
        cmd_vel_limiter_input_topic = f'/{robot_id}/cmd_vel_limiter_input'
        self.create_subscription(
            Twist,
            cmd_vel_limiter_input_topic,
            lambda msg, rid=robot_id: self._cmd_vel_limiter_cb(rid, msg),
            10,
        )
        cmd_vel_topic = f'/{robot_id}/cmd_vel'
        self.cmd_vel_pubs[robot_id] = self.create_publisher(Twist, cmd_vel_topic, 10)
    ...
    def _cmd_vel_limiter_cb(self, robot_id: str, msg: Twist) -> None:
        """Nav2/velocity_smoother 출력을 받아 평소 0.3 m/s, 운반 중 0.25 m/s로 제한해 cmd_vel로 발행."""
        pub = self.cmd_vel_pubs.get(robot_id)
        if pub is None:
            return
        state = self.robot_states.get(robot_id)
        max_vel = 0.25 if (state and state.carried_weight > 0.0) else 0.3
        out = Twist()
        out.linear.x = max(-max_vel, min(max_vel, float(msg.linear.x)))
        ...
        if base == 'task_started':
            ...
            if task_id_val is not None and task_id_val != -1:
                ...
                if is_retry:
                    ...
                    # # self._hide_all_cargo(robot_id)  # Cargo visualization disabled
                    self.task_progress[progress_key] = {"picks": set(), "drops": set()}
            ...
            if current_idx < len(seq):
                ...
                if expected_task.task_id == task_id_val:
                    ...
                    # Task doesn't match - could be a retry of failed task
                    # Check if this is a retry by looking backwards in sequence
                    is_retry = any(t.task_id == task_id_val for t in seq[:current_idx])
                    rid_str = self._color_robot(robot_id)
                    if is_retry:
                        self.get_logger().info(
                            f"[scheduler] {rid_str} task {task_id_val} retry: started but sequence already at index {current_idx} (expected {expected_task.task_id})"
                        )
                    else:
                        self.get_logger().warn(
                ...
                for t in active_seq:
                    if t.task_id == task_id_val:
                        ...
                        if pick_idx in progress["picks"]:
                            ...
                            # Get weight from pick_weights or use task type weight
                            if hasattr(t, 'pick_weights') and t.pick_weights and pick_idx < len(t.pick_weights):
                                ...
                                # Use task type specific weight
                                weight = self.get_item_weight(t.task_type, pick_idx)
                            
                            state.carried_weight += weight
                            # self._set_cargo_visible(robot_id, "big", True)
                            # (cargo visibility disabled)
                            ...
                            # Spawn cargo box on first pick
                            # Cargo box: 로봇 SDF에 cargo_visual 링크로 붙어 있음 (Gazebo에서 자동 이동)
                        ...
                        if t.task_id == task_id_val:
                            ...
                            if hasattr(t, 'deliveries') and t.deliveries and wp_idx < len(t.deliveries) and len(t.deliveries[wp_idx]) > 0:
                                ...
                                if hasattr(t, 'pick_weights') and t.pick_weights and len(t.pick_weights) > 0:
                                    ...
                                    # Use task type specific weight for first pick (FIFO)
                                    dropped_weight = self.get_item_weight(t.task_type, 0)
                            
                            state.carried_weight = max(0.0, state.carried_weight - dropped_weight)
                            # self._set_cargo_visible(robot_id, "big", state.carried_weight > 0.0)
                            # (cargo visibility disabled)
                            ...
                            # Delete cargo box on drop (PICK_DROP/DUAL_PICK_MULTI_DROP typically have one drop)
                            num_drops = len(t.drops) if t.drops else 0
                            # Cargo box: 로봇 SDF cargo_visual 링크 사용 (삭제 불필요)
                        ...
                        if absolute_wp_idx < num_picks:
                            ...
                            if pick_idx in progress["picks"]:
                                ...
                                if hasattr(t, 'pick_weights') and t.pick_weights and pick_idx < len(t.pick_weights):
                                    ...
                                    if pick_idx == 0:
                                        ...
                                        # self._set_cargo_visible(robot_id, "small", True)
                                        pass
                                    progress["picks"].add(pick_idx)
                                    self.task_progress[progress_key] = progress
                                    ...
                                    # Spawn cargo box on first pick
                                    # Cargo box: 로봇 SDF cargo_visual 링크 사용
                            ...
                            if drop_idx in progress["drops"]:
                                ...
                                if hasattr(t, 'deliveries') and t.deliveries and drop_idx < len(t.deliveries):
                                    ...
                                    # Use task type specific weight for the drop index
                                    dropped_weight = self.get_item_weight(t.task_type, drop_idx)
                                
                                state.carried_weight = max(0.0, state.carried_weight - dropped_weight)
                                if drop_idx == 0:
                                    # self._set_cargo_visible(robot_id, "big", False)
                                    pass
                                else:
                                    # self._set_cargo_visible(robot_id, "small", False)
                                    pass
                                ...
                                # For MULTI_PICK_DROP: drop big box first, then small box
                                # Cargo box: 로봇 SDF cargo_visual 링크 사용 (삭제 불필요)
            ...
            if pub is None:
                ...
                if is_charge_fail:
                    ...
                    if rc >= self.retry_limit:
                        ...
                        if ch is not None:
                            # Charge duration proportional to remaining capacity at retry time
                            cap_wh = self.robot_cap_wh.get(robot_id, 120.0)
                            remaining_frac = max(0.0, (cap_wh - state.soc) / cap_wh) if cap_wh > 0.0 else 0.0
                            charge_rate_frac_per_sec = 0.04
                            if remaining_frac <= 0.0:
                                charge_duration_s = 0.0
                            else:
                    ...
                    if failed_task is not None:
                        # Perform Nav2 recovery steps before retrying (if enabled)
                        if self.enable_nav2_recovery:
                            self._recover_nav2_for_robot(robot_id, state)
                        else:
                            rid_str = self._color_robot(robot_id)
                            self.get_logger().info(
                                f"[scheduler] {rid_str} Nav2 recovery disabled - skipping recovery, reassigning task"
                            )
                        ...
                        if not waypoints:
                            ...
                            if start_idx >= len(waypoints):
                                ...
                                if failed_task.task_type == "PICK_DROP":
                                    ...
                                    # If drop is already completed, task should be done - skip retry and advance sequence
                                    if drop_completed:
                                        state.available = True
                                        cur_idx = self.current_seq_index.get(robot_id, 0)
                                        if cur_idx < len(seq):
                                            self.current_seq_index[robot_id] = cur_idx + 1
                                        self.get_logger().warn(
                                            f"[scheduler] {rid_str} task {failed_tid} retry: drop already completed, marking task done (seq +1)"
                                        )
                                    ...
                                    # skip to drop waypoint (index 1) since pick operation already finished
                                        ...
                                        if len(waypoints) > 1:
                                            ...
                                            # No more waypoints, task should be done - skip retry and advance sequence
                                            state.available = True
                                            cur_idx = self.current_seq_index.get(robot_id, 0)
                                            if cur_idx < len(seq):
                                                self.current_seq_index[robot_id] = cur_idx + 1
                                            self.get_logger().warn(
                                                f"[scheduler] {rid_str} task {failed_tid} retry: pick completed but no drop waypoint, marking done (seq +1)"
                                            )
                                    ...
                                    # If all drops are completed, task should be done - advance sequence
                                    if len(progress["drops"]) >= 2:
                                        ...
                                        if cur_idx < len(seq):
                                            self.current_seq_index[robot_id] = cur_idx + 1
                                        self.get_logger().warn(
                                            f"[scheduler] {rid_str} task {failed_tid} retry: all drops already completed, marking task done (seq +1)"
                                        )
                                    else:
                                ...
                                # Only proceed with retry if task is not done (state.available was not set to True above)
                                if not state.available:
                                    ...
                                    state.available = False
                                    self.get_logger().warn(
                                        f"[scheduler] {rid_str} task_failed for id={failed_tid}: resumed at waypoint {start_idx} "
                                        f"(retrying, total waypoints={len(waypoints)})"
                                    )
                    else:
                        state.available = True
                        self.get_logger().warn(
    ...
    def _goal_viz_timer_cb(self) -> None:
        ...
        for rid in self.robot_ids:
            ...
            # Task ID text marker above robot (shows current task_id status)
            task_id = self.current_task_id.get(rid)
            if task_id is None:
                task_text = "IDLE"
            elif task_id == -1:
                task_text = "CHARGE"
            ...
            task_marker.header.stamp = now
            task_marker.ns = f'task_id_{rid}'
            task_marker.id = 0
            task_marker.type = Marker.TEXT_VIEW_FACING
            task_marker.action = Marker.ADD
            # Position text above the robot
            vx, vy = self.viz_pose.get(rid, (state.x, state.y))
            task_marker.pose.position.x = vx
            task_marker.pose.position.y = vy
            task_marker.pose.position.z = 0.5  # Above robot
            task_marker.pose.orientation.w = 1.0
            task_marker.text = task_text
            task_marker.scale.z = 0.3  # Font size
            task_marker.color.r = float(r[0])
            task_marker.color.g = float(r[1])
            task_marker.color.b = float(r[2])
            task_marker.color.a = 1.0
            task_marker.lifetime.sec = 1
    ...
    def _task_id_display_timer_cb(self) -> None:
        """Update task_id display (disabled - not spawning models)."""
        pass

    # def _set_cargo_visible(self, robot_id: str, box_key: str, visible: bool) -> None:
    #     """로봇 cargo_visual 표시 여부 퍼블리시 (값이 바뀔 때만). box_key: big|small."""
    #     pubs = self._cargo_visible_pubs.get(robot_id)
    #     if not pubs or box_key not in pubs:
    #         return
    #     last = self._cargo_visible_last.get(robot_id, {}).get(box_key, None)
    #     if last == visible:
    #         return
    #     self._cargo_visible_last[robot_id][box_key] = visible
    #     msg = Bool()
    #     msg.data = visible
    #     pubs[box_key].publish(msg)

    # def _hide_all_cargo(self, robot_id: str) -> None:
    #     """모든 cargo 박스 숨김 (big/small)."""
    #     # self._set_cargo_visible(robot_id, "big", False)
    #     # self._set_cargo_visible(robot_id, "small", False)

    def _update_gazebo_task_id(self, robot_id: str, task_id: Optional[int]) -> None:
        """Update the task_id display in Gazebo for the given robot.
        
        This spawns/updates a text model above the robot showing the current task status.
        """
        if self.spawn_entity_client is None or self.delete_entity_client is None:
            return
        ...
        if task_id is None:
            ...
            text_label = f"Task {task_id}"
        
        # Model name for this robot's task label
        model_name = f"{robot_id}_task_id"
        
        # Delete old model if exists
        if model_name in self.gazebo_task_models:
            try:
                ...
                self.get_logger().debug(f"Failed to delete old task_id model {model_name}: {e}")
        
        # Create SDF for text display using simple box with label
        sdf_string = f'''
<sdf version='1.7'>
  <model name='{model_name}'>
    <pose>{state.x:.2f} {state.y:.2f} 0.5 0 0 0</pose>
    <static>true</static>
    <link name='link'>
      <visual name='visual'>
        <geometry>
...
'''
        ...
        try:
            ...
            req.xml = sdf_string
            req.robot_namespace = ''
            req.initial_pose.position.x = state.x
            req.initial_pose.position.y = state.y
            req.initial_pose.position.z = 0.5
            req.initial_pose.orientation.w = 1.0
            
            self.spawn_entity_client.call_async(req)
            self.gazebo_task_models[model_name] = model_name
        except Exception as e:
            self.get_logger().debug(f"Failed to spawn task_id model {model_name}: {e}")
    ...
    def _maybe_issue_online_charge(self, rid: str, state: RobotState) -> bool:
        ...
        # charge for a duration proportional to remaining capacity.
