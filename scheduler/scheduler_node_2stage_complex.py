...
from mealpy.swarm_based import MPA, HHO
...
class SchedulerNode(Node):
    ...
    def __init__(self) -> None:
        ...
        # its target (used for Nav2 retry heuristics).
        self.reach_distance_threshold = 0.5
        ...
        # Per-robot current task sequence (offline optimized plan) and
        # index into that sequence. Initialized empty and filled by
        # _offline_initialize_sequences().
        ...
        # ------------------------------------------------------------------
        # Per-robot runtime / makespan tracking
        # ------------------------------------------------------------------
        # Time when each robot first starts executing any task in its
        # offline sequence, and when it finishes its last task.
        self.robot_start_time = {rid: None for rid in self.robot_ids}
        self.robot_finish_time = {rid: None for rid in self.robot_ids}
        # Robots that are considered "dead" for this run: spawned with a
        ...
        # support ANSI escape sequences). Supports up to 10 robots.
        ...
        # ------------------------------------------------------------------
        # Energy model parameters (shared across robots)
        # ------------------------------------------------------------------
        ...
        self.base_robot_mass_kg = 5.0  # Typical TurtleBot3 mass (~5kg)
        ...
        # Robots consume energy even when stationary (computers, sensors, etc.)
        self.E_static_wh_per_s = 0.001  # 0.001 Wh/s = 3.6 W
        # Store last odom timestamp per robot for calculating static energy
        self._last_odom_time: Dict[str, float] = {}

        # Real-world energy consumption noise/uncertainty (sigma)
        self.energy_noise_sigma = 0.01  # Standard deviation of energy noise (Wh)

        # Deceleration energy handling parameters
        self.deceleration_policy = "none"  # "none", "regenerative", "braking_loss"
        self.regenerative_efficiency = 0.0
        self.braking_loss_coefficient = 0.1

        # ------------------------------------------------------------------
        # Per-robot battery capacities (Wh)
        # ------------------------------------------------------------------
        ...
        self.initial_soc_factor: float = 0.25

        # ------------------------------------------------------------------
        # Chargers and Nav2 path planning interfaces
        # ------------------------------------------------------------------
        ...
        # Gazebo ground truth: robot_id -> (x, y, yaw)
        self.gazebo_ground_truth: Dict[str, Tuple[float, float, float]] = {}

        # ------------------------------------------------------------------
        # Per-robot interfaces (pose/odom/task events/Nav2 recovery)
        # ------------------------------------------------------------------
        ...
        # ------------------------------------------------------------------
        # Optimization and charging strategy parameters
        # ------------------------------------------------------------------
        # Optimization parameters (algorithm selection removed - now runs all 3 algorithms)
        self.optimization_epoch = self.declare_parameter("optimization_epoch", 300).get_parameter_value().integer_value
        self.optimization_pop_size = self.declare_parameter("optimization_pop_size", 300).get_parameter_value().integer_value
        ...
        # ------------------------------------------------------------------
        # Timers: main scheduling loop, SOC logs, goal visualization, health
        # ------------------------------------------------------------------
        ...
        self.get_logger().info(
            f"SchedulerNode (complex) started for robots={self.robot_ids}; "
            f"task allocation: running GA, MPA, HHO and selecting best by makespan "
            f"(epoch={self.optimization_epoch}, pop_size={self.optimization_pop_size})"
        )

        # Health check timer: after a grace period from node startup, detect
        # robots that never started any task even though they have a non-empty
        # offline sequence. Mark them as dead and log once.
    ...
    # ------------------------------------------------------------------
    # Path planning helper for accurate distance calculation (Nav2 based)
    # ------------------------------------------------------------------
    ...
    def _optimized_charging_for_all_robots(
        self,
        ...
        This mirrors the legacy old_scheduler_node model:
          - Decision variable per (robot, task_index): which charger to use or no-charge
          - Hard constraint: no SOC depletion
          - Soft objectives: makespan, waiting at chargers, extra drive time,
            number of charges, and temporal overlap on the same charger.
        """
        ...
        index_map: List[Tuple[str, int]] = []  # list of (rid, task_index)
        ...
        def _objective(solution: np.ndarray) -> float:
            ...
            for rid in robot_ids:
                ...
                for i, task in enumerate(seq):
                    ...
                    if decision >= 0 and chargers_list:
                        ...
                        # second in normalized units, capped at full.
            ...
            # seen so far in this optimization run.
        ...
        epoch = getattr(self, "optimization_epoch", 300)
        pop_size = getattr(self, "optimization_pop_size", 60)
        model = SMA.OriginalSMA(epoch=epoch, pop_size=pop_size, pr=0.03)

        self.get_logger().info(
            f"[optimization-stage2] Starting charging optimization (SMA): epoch={epoch}, pop_size={pop_size}, slots={K}"
        )
        ...
        try:
            ...
            self.get_logger().error(
                f"[optimization-stage2] SMA charging optimization failed: {type(e).__name__}: {e}"
            )
            return {rid: list(base_sequences.get(rid, [])) for rid in self.robot_ids}

        if best is None or not hasattr(best, "solution"):
            self.get_logger().error(
                "[optimization-stage2] SMA charging optimization returned invalid solution"
            )
            return {rid: list(base_sequences.get(rid, [])) for rid in self.robot_ids}
        ...
        if actual_size != K:
            self.get_logger().warn(
                f"[optimization-stage2] Solution size mismatch: expected {K}, got {actual_size}"
            )

        # Decode best solution to final augmented sequences
        best_plan = _decode_solution_to_plan(solution)
        chargers_list = list(self.chargers)
    ...
    def _recover_nav2_for_robot(self, robot_id: str, state: RobotState) -> None:
        ...
        try:
            ...
            if gazebo_gt is None:
                # self.get_logger().warn(
                #     f"[nav2-recovery] {rid_str}: No Gazebo ground truth (/gazebo/model_states) yet, using fallbacks if needed"
                # )
                ...
                if gazebo_gt is None:
                    # self.get_logger().warn(
                    #     f"[nav2-recovery] {rid_str}: Gazebo ground truth unavailable, switching to TF fallback"
                    # )
                    ...
                    try:
                        ...
                        # self.get_logger().warn(
                        #     f"[nav2-recovery] {rid_str}: TF fallback pose from state: ({map_x:.3f}, {map_y:.3f}), yaw={math.degrees(map_yaw):.1f}°"
                        #         )
                    except Exception as e:
                        # self.get_logger().warn(
                        #     f"[nav2-recovery] {rid_str}: TF lookup failed ({type(e).__name__}), using state.x,y as map pose"
                        # )
                        ...
                        gazebo_gt = (map_x, map_y, map_yaw)
                        # self.get_logger().warn(
                        #     f"[nav2-recovery] {rid_str}: Local fallback pose from state: ({map_x:.3f}, {map_y:.3f}), yaw=0°"
                        # )
    ...
    def _feasibility_charging_for_all_robots(
        ...
        for rid in self.robot_ids:
            ...
            for task in seq:
                ...
                # 1) Estimate energy from current pose to execute this task.
                try:
                    required, _ = self.estimate_task_energy(
                        RobotState(
                            robot_id=rid,
                ...
                # 2) If SOC is not enough, insert a CHARGE task *before* this task.
                if soc < required:
                    # Choose nearest charger purely by distance (offline feasibility),
                    # not by runtime queue state.
                    try:
                        charger = self._nearest_charger(x, y)
                    except Exception:
                        charger = None

                    if charger is None:
                        # No charger defined so cannot make this sequence feasible.
                        # Just append the remaining tasks as-is and stop adding
                        # further CHARGE events for this robot.
                        augmented[rid].append(task)
                        continue

                    # Estimate energy needed to reach the charger from current pose.
                    try:
                        e_to_ch, _ = self.estimate_task_energy(
                            RobotState(
                                robot_id=rid,
                                namespace=state0.namespace,
                                x=x,
                    ...
                    if soc < e_to_ch:
                        # Cannot even reach the charger with current SOC → infeasible.
                        augmented[rid].append(task)
                        continue

                    # Insert CHARGE task: move to charger, then recharge to full.
                    charge_task = TaskSpec(
                        task_id=-1,
                        task_type="CHARGE",
                        picks=[(x, y)],
                        ...
                        pick_wait_s=0.0,
                        drop_wait_s=0.0,
                        # Offline model: we don't simulate detailed time here; duration
                        # is handled by the runtime charging model. Use 1s placeholder.
                        charge_duration_s=1.0,
                    )
                    augmented[rid].append(charge_task)

                    # Pay energy to reach charger, then recharge to full.
                    soc -= e_to_ch
                    if soc < 0.0:
                        soc = 0.0
                    ...
                    # Re-estimate required energy for the same task from charger pose.
                    try:
                        required, _ = self.estimate_task_energy(
                            RobotState(
                                robot_id=rid,
                ...
                # 3) Execute the task in the offline model (after optional charging).
                if soc < required:
                    # Even after charging, still not enough → mark infeasible but
                    # keep the task in sequence so that executor can handle it.
                    self.get_logger().warn(
                        f"[feasibility] {rid} SOC={soc:.2f}Wh still < required={required:.2f}Wh "
                        f"for task {task.task_id}; appending task without additional CHARGE."
                    )
                else:
                    soc -= required
    ...
    def _optimized_charging_for_all_robots(
        ...
        """Stage-2: optimized multi-robot charging given fixed task sequences.

        Stage 1 has already decided which robot does which tasks and in what
        order (base_sequences). Here we optimize only the binary decision of
        whether to insert a CHARGE before each task, using a Mealpy optimizer
        and the same objective structure as the 1-stage GA (makespan,
        energy violations, charging time/count, and imbalance).
        """

        robot_list = list(self.robot_ids)

        # Flatten task positions into a 1D index for the optimizer.
        slot_indices: Dict[str, List[int]] = {rid: [] for rid in robot_list}
        slot_tasks: List[Tuple[str, TaskSpec]] = []
        ...
        for rid in robot_list:
            ...
            for task in seq:
                idx = len(slot_tasks)
                slot_tasks.append((rid, task))
                slot_indices[rid].append(idx)

        N = len(slot_tasks)
        if N == 0:
            return {rid: list(base_sequences.get(rid, [])) for rid in robot_list}

        num_chargers = max(1, len(self.chargers))
        chargers_list = list(self.chargers)

        def _objective(solution: np.ndarray) -> float:
            # Each gene is a charge_key in [-1, 1] for a fixed task slot.
            # charge_key > 0.0 → insert CHARGE before that task.
            ...
            # Global event list for realistic charger contention
            ...
            for rid in robot_list:
                ...
                base_seq = base_sequences.get(rid, [])
                if not base_seq:
                    continue
                ...
                slot_idx_list = slot_indices.get(rid, [])
                if len(slot_idx_list) != len(base_seq):
                    # Defensive: mismatch between slots and tasks
                    total_cost += 1e6
                    continue

                charge_keys = [float(solution[i]) for i in slot_idx_list]
                seq = list(base_seq)
                ...
                for task_idx, t in enumerate(seq):
                    charge_key = charge_keys[task_idx]
                    ...
                    if should_charge:
                        ...
                        reachable_chargers_list: List[Tuple[int, float, float]] = []
                        ...
                        robot_time = best_charge_end
                        energy = cap_wh

                        charge_events.append(
                            (rid, task_idx, reachable_chargers_list, time_before_charge, best_charge_end, charge_duration)
                        )
            ...
            # Phase 2: resolve charger contention across all robots.
            ...
            lambda_charge_time = 0.1
            lambda_charge_count = 20.0
            lambda_balance = 1.0
        ...
        problem = {
            "obj_func": _objective,
            "bounds": FloatVar(lb=(-1.0,) * N, ub=(1.0,) * N),
            "minmax": "min",
            "log_to": None,
        }
        ...
        self.get_logger().info(
            f"[optimization-stage2] Starting charging optimization (GA): epoch={epoch}, pop_size={pop_size}, slots={N}"
        )
        ...
        try:
            ...
            self.get_logger().error(
                f"[optimization-stage2] GA charging optimization failed: {type(e).__name__}: {e}"
            )
            return {rid: list(base_sequences.get(rid, [])) for rid in robot_list}

        if best is None or not hasattr(best, "solution"):
            self.get_logger().error(
                "[optimization-stage2] GA charging optimization returned invalid solution"
            )
            return {rid: list(base_sequences.get(rid, [])) for rid in robot_list}
        ...
        if actual_size != N:
            self.get_logger().warn(
                f"[optimization-stage2] Solution size mismatch: expected {N}, got {actual_size}"
            )

        # Decode: build augmented sequences with CHARGE events, mirroring
        # the 1-stage GA's decoding logic so that offline plan matches the
        # energy/charger contention model used in the objective.
        num_chargers = max(1, len(self.chargers))
        chargers_list = list(self.chargers)
        ...
        augmented: Dict[str, List[TaskSpec]] = {}
        robot_times: Dict[str, float] = {}
        max_makespan = 0.0
        ...
        for rid in robot_list:
            ...
            idx_list = slot_indices.get(rid, [])
            if len(idx_list) != len(base_seq):
                augmented[rid] = list(base_seq)
                robot_times[rid] = 0.0
                continue

            charge_keys = [float(solution[i]) for i in idx_list]
            seq = list(base_seq)
            ...
            augmented[rid] = augmented_seq
            robot_times[rid] = robot_time
            max_makespan = max(max_makespan, robot_time)

        self.get_logger().info(
            f"[optimization-stage2] Charging optimization completed: makespan={max_makespan:.2f}s"
        )
    ...
    def _init_demo_tasks(self) -> None:
        ...
        # Build demo tasks solely from demo_test (simple PICK_DROP tasks)
        # 기존 demo_specs, demo_pairs_multi, demo_pairs 기반 로직은 사용하지 않고
        # demo_test 좌표 목록만으로 태스크 풀을 구성한다.
        self.pending_tasks = []
        tid = 1

        for (px, py, dx, dy) in demo_test:
    ...
    def _optimize_with_algorithm(self, candidate_tasks: List[TaskSpec], algorithm_name: str) -> Tuple[Dict[str, List[TaskSpec]], float]:
        ...
        Stage 1 (tasks only):
        - Optimizes which robot gets which task (allocation)
        - Optimizes the order of tasks for each robot (sequencing)
        - Does *not* optimize charging decisions (no CHARGE in the genome)

        Encoding (per task):
        - robot_key: mapped from [-1.0, 1.0] → robot index [0, num_robots)
        - seq_key: random key used to sort tasks within each robot

        Returns:
            (Dict[robot_id, List[TaskSpec]], max_makespan_without_charging)
        """
        ...
        # ---------- objective (Stage 1: tasks only, no charging) ----------
        def _objective_function(solution: np.ndarray) -> float:
            """Objective for Stage 1 (task allocation + sequencing only).

            - Genome encodes only robot assignment and sequence keys.
            - Charging is NOT modeled here; energy is checked approximately
              and violations are penalized softly so that Stage 2 can later
              insert CHARGE tasks to restore feasibility.
            """

            # Decode solution: [robot_key_1, seq_key_1, robot_key_2, seq_key_2, ...]
            # Each task has 2 values: robot assignment and sequence key
            assignments: Dict[str, List[Tuple[float, TaskSpec]]] = {rid: [] for rid in robot_list}

            for i, task in enumerate(candidate_tasks):
                robot_key = solution[2 * i]
                seq_key = solution[2 * i + 1]
                ...
                assignments[robot_id].append((seq_key, task))
            ...
            for rid in robot_list:
                ...
                state = self.robot_states.get(rid)
                if state is None:
                    # Robot not available → count as a violation
                    total_penalty += 1.0
                ...
                for t in seq:
                    try:
                        ...
                        # Soft penalty: energy estimation failed
                        required_energy, t_to_pick = 0.0, 0.0
                        penalty += 1.0
                        continue

                    if energy < required_energy:
                        # Soft penalty: insufficient energy without charging in Stage 1
                        penalty += 1.0
                        # Stop further accumulation for this robot; Stage 2 will add charging
                        break

                    # Execute task in the Stage 1 model (no charging)
                    energy -= required_energy
                    robot_time += t_to_pick
            ...
            # - max_makespan : completion time without charging
            # - total_penalty: number of energy/estimation violations
            # - imbalance    : load imbalance across robots
        ...
        # ---------- mealpy problem ----------
        # Solution dimension: 2 * K (robot_key + seq_key for each task)
        problem = {
            "obj_func": _objective_function,
            "bounds": FloatVar(lb=(-1.0,) * (2 * K), ub=(1.0,) * (2 * K)),
            "minmax": "min",
            "log_to": None,
        }

        # Create optimizer based on algorithm_name with per-algorithm settings
        base_epoch = self.optimization_epoch
        base_pop_size = self.optimization_pop_size

        if algorithm_name == 'GA':
            # GA는 전역 탐색과 수렴 성능이 좋아서 전체 epoch/pop_size를 그대로 사용
            epoch = base_epoch
            pop_size = base_pop_size
            model = GA.OriginalGA(epoch=epoch, pop_size=pop_size, pc=0.9, pm=0.05)
        elif algorithm_name == 'MPA':
            # MPA는 세대당 연산량이 커서 더 가볍게 돌려야 전체 시간이 폭발하지 않음
            epoch = max(50, base_epoch // 3)
            pop_size = max(50, base_pop_size // 10)
            model = MPA.OriginalMPA(epoch=epoch, pop_size=pop_size)
        elif algorithm_name == 'HHO':
            # HHO도 탐색 비용이 크므로, 보조 탐색용으로 축소된 설정 사용
            epoch = max(50, base_epoch // 3)
            pop_size = max(50, base_pop_size // 3)
            model = HHO.OriginalHHO(epoch=epoch, pop_size=pop_size)
        else:
            # 알 수 없는 알고리즘 이름이면 GA 기본 설정으로 폴백
            self.get_logger().warn(
                f"[optimization] Unknown algorithm '{algorithm_name}', falling back to GA"
            )
            epoch = base_epoch
            pop_size = base_pop_size
            model = GA.OriginalGA(epoch=epoch, pop_size=pop_size, pc=0.9, pm=0.05)
        ...
        # ---------- decode best solution (Stage 1: tasks only) ----------
        solution = best.solution

        # Verify solution size matches expected dimension
        expected_size = 2 * K  # [robot_key, seq_key] per task
        actual_size = len(solution) if hasattr(solution, "__len__") else 0
        ...
        for i, task in enumerate(candidate_tasks):
            if 2 * i + 1 >= len(solution):
                self.get_logger().warn(
                    f"[optimization] Solution too short for task {i}: need index {2 * i + 1}, have {len(solution)}"
                )
                break
        ...
        # Build per-robot sequences (no CHARGE) and estimate approximate times
        ...
        for rid in robot_list:
            ...
            # For ETA in the assignment log, we approximate completion time
            # by summing per-task durations for the full sequence, without
            # truncating based on current SOC. Charging is handled separately
            # by the chosen charging strategy.
            for t in seq:
                try:
                    _, t_task = self.estimate_task_energy(
                        RobotState(
                            robot_id=sim_robot.robot_id,
                            namespace=sim_robot.namespace,
                            x=sim_robot.x,
                    ...
                    # Ignore failures here; this is only for logging ETA
                    t_task = 0.0
    ...
    def _optimize_multi_robot_allocation(self, candidate_tasks: List[TaskSpec]) -> Dict[str, List[TaskSpec]]:
        """Run multiple optimizers (GA, MPA, HHO) and pick the best result.

        All algorithms share the same Stage-1 objective. We run them
        sequentially with the configured epoch/pop_size and select the
        allocation with the smallest estimated makespan.
        """
        if not candidate_tasks:
        ...
        algorithms = ['GA']
        best_result: Dict[str, List[TaskSpec]] = {rid: [] for rid in self.robot_ids}
        best_makespan = float('inf')
        best_algo = None

        for algo_name in algorithms:
            try:
                self.get_logger().info(f"[multi-algo] Running {algo_name}...")
        ...
        if best_algo is None:
            self.get_logger().error("[multi-algo] All optimizers failed; returning empty allocation")
            return {rid: [] for rid in self.robot_ids}

        self.get_logger().info(
            f"[multi-algo] Selected {best_algo} with best makespan={best_makespan:.2f}s"
        )
    ...
    def _offline_initialize_sequences(self) -> None:
        ...
        # IMPORTANT: We no longer reset SOC to 100% here; offline planning
        # (both Stage 1 and Stage 2) should reflect the robots' *actual*
        # remaining energy so that CHARGE insertion (feasibility / optimized)
        # matches what will happen online.
        self.optimization_in_progress = True

        self.get_logger().info(
            "[optimization] Starting precompute and allocation - energy consumption paused "
            "until allocation completes (SOC left unchanged for realistic feasibility)"
        )
        ...
        # ------------------------------------------------------------------
        # Diagnostic: hypothetical offline CHARGE insertion (feasibility model)
        #
        # For charge=feasibility/threshold we still execute purely online
        # charging, but for debugging it is useful to know where a greedy
        # feasibility-based offline planner *would* insert CHARGE (-1).
        #
        # This does NOT affect the actual offline sequences used at
        # runtime; we only log the hypothetical indices and task-id
        # sequences with CHARGE included so you can inspect them.
        # ------------------------------------------------------------------
        if self.charging_strategy in ("feasibility", "threshold"):
            try:
                debug_augmented = self._feasibility_charging_for_all_robots(base_sequences)
                for rid in self.robot_ids:
                    dbg_seq = debug_augmented.get(rid, [])
                    if not dbg_seq:
                        continue

                    charge_indices = [i for i, t in enumerate(dbg_seq) if getattr(t, "task_type", "") == "CHARGE" or getattr(t, "task_id", 0) == -1]
                    if not charge_indices:
                        continue

                    seq_ids_dbg = [getattr(t, "task_id", 0) for t in dbg_seq]
                    self.get_logger().info(
                        f"[offline-debug] {rid}: hypothetical CHARGE insertion (feasibility model) -> "
                        f"charge_indices={charge_indices}, seq_with_charge={seq_ids_dbg}"
                    )
            except Exception as e:
                self.get_logger().warn(
                    f"[offline-debug] feasibility-based CHARGE simulation failed: {type(e).__name__}: {e}"
                )

        # Stage 2: charging decisions, mode-dependent
        #
        # - "optimized"   : run a GA-based offline charging optimizer that
        #                    chooses where to insert CHARGE before each task,
        #                    using the same objective as the 1-stage GA.
        # - "feasibility" : keep offline sequences CHARGE-free and rely on
        #                    online feasibility logic (_maybe_issue_online_charge)
        #                    exactly like the old 2-stage implementation.
        # - "threshold"   : keep offline sequences CHARGE-free and use the
        #                    SOC-threshold-based online policy only.

        if self.charging_strategy == "optimized":
            augmented_sequences = self._optimized_charging_for_all_robots(base_sequences)
        else:
            # feasibility / threshold / any future online-only strategies:
            # offline plan contains only PICK/DROP-type tasks; CHARGE is
            # inserted at runtime by the online charging policy.
    ...
    def _task_event_cb(self, robot_id: str, msg: String) -> None:
        ...
        if base == 'task_started':
            ...
            # Ensure task_type is always defined in this scope so that
            # later logging does not hit UnboundLocalError even for CHARGE tasks.
            task_type = "UNKNOWN"
            ...
            if task_id_val == -1:
                ...
                for t in active_seq:
                    if t.task_type == "CHARGE":
                        ...
                        break
                    # For logging purposes below, mark this as a CHARGE task
                    task_type = "CHARGE"
            ...
            if task_id_val is not None and task_id_val != -1:
                ...
                # Find task type for logging (overrides default/CHARGE type)
                active_seq = self.current_sequences.get(robot_id, [])
                for t in active_seq:
                ...
                if is_retry:
                    ...
                    # Detailed retry log is now omitted to keep logs concise
                else:
            ...
            if current_idx < len(seq):
                ...
                if expected_task.task_id == task_id_val:
                    ...
                    # Compact one-line log with SOC and progress (k/N)
                    rid_str = self._color_robot(robot_id)
                    total_len = len(seq)
                    new_idx = self.current_seq_index[robot_id]
                    progress_str = f"{new_idx}/{total_len}" if total_len > 0 else "0/0"
                    self.get_logger().info(
                        f"[task] {rid_str} start id={task_id_val} type={task_type} soc={state.soc:.2f}Wh progress=({progress_str})"
                    )
                else:
            ...
            if task_id_val is not None and task_id_val != -1:
                ...
                rid_str = self._color_robot(robot_id)
                # Compact one-line completion log with SOC and progress
                seq = self.current_sequences.get(robot_id, [])
                current_idx = self.current_seq_index.get(robot_id, 0)
                total_len = len(seq)
                progress_str = f"{min(current_idx, total_len)}/{total_len}" if total_len > 0 else "0/0"
                self.get_logger().info(
                    f"[task] {rid_str} done id={task_id_val} type={task_type} soc={state.soc:.2f}Wh progress=({progress_str})"
                )
    ...
    def _timer_cb(self) -> None:
        ...
        for rid in self.robot_ids:
            ...
            # For feasibility mode, track the first index in the offline
            # sequence at which the robot would no longer have enough
            # SOC to execute the *next* task given the current SOC.
            # This is purely diagnostic and is reflected only in the
            # logged sequence (by inserting a -1 marker), it does not
            # modify the actual current_sequences used for execution.
            first_infeasible_idx: Optional[int] = None
            ...
            for t in seq_specs:
                if t.task_type in ("PICK_DROP", "MULTI_PICK_DROP", "DUAL_PICK_MULTI_DROP"):
                    ...
                    # Detect the first point where sim_energy is not
                    # sufficient to execute the next task for the
                    # feasibility strategy. We intentionally do this
                    # check *before* subtracting e_total so that it
                    # reflects "can I do this upcoming task with my
                    # current SOC?" rather than after-the-fact.
                    if (
                        self.charging_strategy == "feasibility"
                        and first_infeasible_idx is None
                        and sim_energy < e_total
                    ):
                        first_infeasible_idx = len(energy_vals)
            ...
            # For feasibility mode, insert a -1 marker in the *logged*
            # sequence at the first infeasible index (if any) so that
            # it is easy to see from the log where a CHARGE would need
            # to be inserted. This does not alter the underlying
            # current_sequences structure.
            if self.charging_strategy == "feasibility" and first_infeasible_idx is not None:
                display_ids: List[int] = []
                for i, tid in enumerate(seq_ids):
                    if i == first_infeasible_idx:
                        display_ids.append(-1)
                    display_ids.append(tid)
                seq_str = ','.join(str(tid) for tid in display_ids)
            else:
                seq_str = ','.join(str(tid) for tid in seq_ids)
